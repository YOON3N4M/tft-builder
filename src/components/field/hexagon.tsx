...
export default function Hexagon(props: HexagonProps) {
  ...
  function handleItemDrop() {
    ...
    if (draggingCoreItem.name.includes("상징")) {
      ...
      // 시너지 중복체크
      const isExist = championWithItem.champion.trait.some(
      ...
      championWithItem.champion.trait.push(synergy);
    }
  ...
  function handleItemRightClick(
    ...
    if (targetItem.name.includes("상징")) {
      const newSynergyList = placedChampion.champion.trait.filter(
        (item) => item.src[0] !== targetItem.src
      );
      ClonedPlacedChampion.champion.trait = newSynergyList;
    }
