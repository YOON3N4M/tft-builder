...
interface ChampionPortraitNewProps extends HTMLAttributes<HTMLDivElement> {
  champion: Champion;
  tooltip?: boolean;
}
...
function ChampionPortraitNew(props: ChampionPortraitNewProps) {
  const {
    champion,
    className,
    children,
    ...
    >
      {tooltip && (
        <PortalTooltip
          ...
          y={pos.y}
        >
          <ChampionTooltipNew champion={champion} />
        </PortalTooltip>
      )}
      ...
      <Image
        ...
        height={128}
        alt={name}
        className={cn("object-cover relative")}
      />
      {children}
    </div>
  );
...
export default ChampionPortraitNew;
