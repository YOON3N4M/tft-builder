...
import { SET_SEASON_LIST, SetSeason } from "@/dataNew";
import { useSetActions } from "@/store/setStore";
...
function SetSeasonSelect() {
  const { setActiveSet } = useSetActions();

  function handleSelectChange(event: ChangeEvent<HTMLSelectElement>) {
    setActiveSet(event.target.value as SetSeason);
  }
  return (
    <div className="flex">
      <span className="text-sub-text">시즌</span>
      <select onChange={handleSelectChange} className="px-md ml-sm">
        {SET_SEASON_LIST.map((set) => (
